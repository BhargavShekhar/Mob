...
export class ArtifactProcessor {
    ...
    parse() {
        const latestActionStart = this.currentArtifact.split("\n").findIndex(line => line.includes("<mobAction type="))
        const latestActionEnd = this.currentArtifact.split("\n").findIndex(line => (
            line.includes("</mobAction>") ?? (this.currentArtifact.split("\n").length-1)
        ))

        if(latestActionStart === -1) return;
        ...
        try {
            if(latestActionType === "\"shell\"") {
                ...
                if(shellCommand.includes("</mobAction>")) {
                    shellCommand = shellCommand.split("</mobAction>")[0]!;
